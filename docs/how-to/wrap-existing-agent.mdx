...
<Tip><Tip>Install the SDK first: `uv add acp-sdk`</Tip></Tip>

Wrap an agent by annotating a Python function with `@server.agent()`. The agent name comes from the function name, and the description comes from the docstring. You can add more metadata like capabilities and dependencies - see the [Agent Manifest](/core-concepts/agent-manifest) section for details.
